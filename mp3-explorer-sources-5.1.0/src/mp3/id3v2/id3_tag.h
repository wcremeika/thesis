...
luint			Link							( const char *fileInfo  , int nMode );
void			Update							( void );
...
void			SetupTag						( const char *fileInfo );
...
uchar			m_nVersion;						// what version tag?
uchar			m_nRevision;						// what revision tag?
...
bool			m_bSyncOn;							// should we unsync this tag when rendering?
bool			m_bCompression;					// should we compress frames when rendering?
bool			m_bPadding;						// add padding to tags?
bool			m_bExtendedHeader;					// create an extended header when rendering?
bool			m_bHasChanged;						// has the tag changed since the last parse or render?
FILE			*fileHandle;					// a handle to the file we are linked to
char			m_strFileName[ 256 ];				// the name of the file we are linked to
char			m_strTempName[ 256 ];				// a temp name in case we need a temp file for copying stuff
luint			m_nFileSize;						// the size of the file (without any tag)
luint			m_nOldTagSize;						// the size of the old tag (if any)
luint			m_nExtraBytes;						// extra bytes to strip from end of file (ID3v1 and Lyrics3 tags)
bool			m_bHasV1Tag;						// does the file have an ID3v1 tag attached?
static luint	instances;						// how many ID3_Tag objects are floating around in this app?

luint	GetFileSize() { return m_nFileSize; };
};
