...
CListViewEx::SetColors(COLORREF clrText, COLORREF clrTextBk, COLORREF clrBkgnd, COLORREF clrHighlight, COLORREF clrHighlightText )
{
	m_clrText = clrText;
	m_clrTextBk = clrTextBk;
	m_clrBkgnd = clrBkgnd;
	m_clrHighlight = clrHighlight;
	m_clrHighlightText = clrHighlightText;
...
// set colors if item is selected
	...
	if (bSelected)
	{
		clrTextSave = pDC->SetTextColor( m_clrHighlightText );
		clrBkSave = pDC->SetBkColor( m_clrHighlight );

		pDC->FillRect(rcAllLabels, &CBrush( m_clrHighlight ));
	}
	else
	{
